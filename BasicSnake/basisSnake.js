...
class NeuralNetwork {
  ...
  run(inputs) {
    ...
    return output; // Devuelve las salidas procesadas
  }
...
function calculateReward(head, food, collision) {
  const prevDistance = Math.sqrt(
      Math.pow(head.prevX - food.x, 2) + Math.pow(head.prevY - food.y, 2)
  );
  ...
  if (collision) {
      return -10; // Penalización por colisión
  } else if (head.x === food.x && head.y === food.y) {
      return +10; // Recompensa por comer la comida
  } else if (currentDistance < prevDistance) {
      return +1; // Recompensa por acercarse a la comida
  } else {
      return -1; // Penalización por alejarse de la comida
  }
}
...
class SnakeBasic {
  ...
  getState() {
    ...
    let dir = this.size;
    // Devuelve el estado del juego como un array (serpiente, comida, dirección)
    return [
      this.pos.x, this.pos.y, // Cabeza de la serpiente
      foodRel.x, foodRel.y,         // Comida
      dir.x, dir.y // Dirección
    ];
  }
  neuralNetworkMovement() {
    // Obtener el estado actual como entrada
    const input = this.getInputArr();

    // Ejecutar la red neuronal para obtener la salida
    const output = net.run(input); // Usa 'run' para obtener predicción
    console.log("Net output:", output);

    // Convertimos la salida a una dirección discreta
    const direction = Math.floor(output[0] * 4); // Salida escalada a 0, 1, 2, 3
    ...
    console.log(a, b);
      return a.map(row => {
          return b[0].map((_, colIndex) => {
              ...
              return row.reduce((sum, elm, i) => sum + elm * b[i][colIndex], 0);
          });
      });
  }
...
const nn = new NeuralNetwork__(6, 12, 4);
...
function trainSnake() {
  // Obtener el estado del juego (por ejemplo, posición de la serpiente, comida y dirección)
  const state = snakeBasic.getState();
  
  // La red neuronal hace una predicción sobre qué acción tomar
  const action = nn.forward(state);
  
  // Las 4 posibles direcciones (arriba, abajo, izquierda, derecha)
  const directions = [
    { x: 0, y: -1 }, // Arriba
    { x: 0, y: 1 },  // Abajo
    { x: -1, y: 0 }, // Izquierda
  ...
  // Elegir la dirección con la mayor salida de la red neuronal
  const chosenDirection = directions[action.indexOf(Math.max(...action))];
  
  // Actualizar la dirección de la serpiente en el juego
  game.setDirection(chosenDirection.x, chosenDirection.y);
  
  // Mover la serpiente y obtener la recompensa
  const reward = game.move();
  
  // Obtener el próximo estado después de mover
  const nextState = game.getState();
  
  // Entrenar la red neuronal con el estado actual y la recompensa obtenida
  const target = Array(4).fill(0);
  target[action.indexOf(Math.max(...action))] = reward;
  
  // Entrenar la red neuronal usando el estado actual y el objetivo calculado
